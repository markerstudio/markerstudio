...
import { EmptyState, Modal } from "@/components/ui/glass";
...
export default function NotesApp({
  ...
  useEffect(() => {
    if (editingId != null) autoGrow(editorBodyRef.current);
  }, [editingId]);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* capture bar — the hero */}
      <section
        onFocus={() => setCFocused(true)}
        onBlur={(e) => {
          if (!e.currentTarget.contains(e.relatedTarget as Node)) setCFocused(false);
        }}
        className={`relative rounded-[26px] transition-all duration-200 ${flash ? "ms-composer-flash" : ""}
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 lq-stagger">
              {wallVisible.map((n, i) => (
                ...
                >
                  ...
                  {!!n.body && (
                    <p className={`text-[13px] leading-relaxed text-charcoal-80 whitespace-pre-wrap line-clamp-6 ${n.title ? "mt-1" : ""}`}>
                      {n.body}
                    </p>
                  )}
        ...
        {editing && (
          <div className="space-y-3">
            <input
              ...
              placeholder="Title"
              className="w-full bg-transparent focus:outline-none font-display font-bold text-[18px] tracking-tight text-ink placeholder:text-charcoal-40/70"
            />
            <textarea
              ref={editorBodyRef}
              value={editing.body}
              ...
              rows={4}
              className="w-full resize-none bg-transparent focus:outline-none text-[15px] leading-relaxed text-charcoal-80 placeholder:text-charcoal-40/70 min-h-[120px]"
            />
