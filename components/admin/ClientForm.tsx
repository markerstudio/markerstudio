  ...
  return (
    <form ref={formRef} action={saveClient} className="space-y-6">
      ...
      <Group title="Marker Plan">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-4">
          <Text label="Notion link (optional)" value={data.plan.notionUrl ?? ""} onChange={(notionUrl) => patch({ plan: { ...data.plan, notionUrl } })} placeholder="https://notion.so/…" />
          <Text label="Money left / balance" value={data.plan.balance ?? ""} onChange={(balance) => patch({ plan: { ...data.plan, balance } })} placeholder="e.g. 3,600 ILS" />
        </div>
      ...
      <Group title="Invoices">
        <Rows<Invoice> items={data.invoices} onChange={(invoices) => patch({ invoices })} blank={{ cycle: "", desc: "", amount: "", status: "due" }} addLabel="Add invoice"
          render={(inv, set) => (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 pr-16">
