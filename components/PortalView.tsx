...
import { generateAiAnalysis } from "@/app/ai-actions";
...
export default function PortalView({
  ...
  async function doGenerateAi() {
    ...
    else setAiErr(r.error || "Generation failed.");
  }
  ...
  return (
    <div className="ms-portal ms-portal--side" dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      {tab === "analysis" && (
        <section className="ms-section ms-rise">
          <div className="ms-container">
            {/* AI strategic reading — generated by Claude from the data below. */}
            {(aiData || canEdit) && (
              <div className="ms-ai" style={{ marginBottom: 28 }}>
                <div className="ms-ai__top">
                  <span className="ms-ai__badge">{ui("AI reading", "قراءة الذكاء الاصطناعي")}</span>
                  {canEdit && (
                    <button className="ms-ai__gen" onClick={doGenerateAi} disabled={aiBusy}>
                      {aiBusy ? ui("Analyzing…", "يحلّل…") : aiData ? ui("Regenerate", "إعادة التوليد") : ui("Generate with AI", "وليّد بالذكاء الاصطناعي")}
                    </button>
                  )}
                </div>
                {aiErr && <p className="ms-ai__err">{aiErr}</p>}
                {aiData ? (
                  <>
                    <h2 className="ms-ai__headline">{tr(aiData.headline)}</h2>
                    <p className="ms-ai__summary" dir="auto">{tr(aiData.summary)}</p>
                    {aiData.insights?.length > 0 && (
                      <div className="ms-ai__insights">
                        {aiData.insights.map((it, i) => (
                          <div key={i} className="ms-ai__insight">
                            <b dir="auto">{tr(it.title)}</b>
                            <span dir="auto">{tr(it.detail)}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    {aiData.recommendations?.length > 0 && (
                      <div className="ms-ai__recs">
                        <span className="ms-ai__recs-label">{ui("What to do next", "الخطوات القادمة")}</span>
                        <ul>
                          {aiData.recommendations.map((r, i) => <li key={i} dir="auto">{tr(r)}</li>)}
                        </ul>
                      </div>
                    )}
                  </>
                ) : (
                  canEdit && <p className="ms-ai__empty">{ui("Let Claude read this client's numbers and write a sharp bilingual analysis.", "دع كلود يقرأ أرقام هذا العميل ويكتب تحليلاً ثنائي اللغة.")}</p>
                )}
              </div>
            )}
